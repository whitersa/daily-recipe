    ...
    <div className="flex flex-col gap-2 mb-1">
      ...
      <div className="flex gap-2 h-10 relative isolate">
        ...
        <button 
          ...
          className="w-10 h-10 flex items-center justify-center bg-white/70 backdrop-blur-xl border border-white/80 rounded-[6px] text-black/60 hover:text-[#1C1C1E] hover:bg-white transition-all active:scale-95 shadow-sm"
        >
      ...
      {isFilterOpen && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 h-[100dvh]">
          ...
          ></div>
          <div className="relative w-full max-w-[480px] bg-white rounded-t-[32px] sm:rounded-[6px] shadow-2xl p-8 pt-10 sm:p-7 animate-slide-up space-y-8 pb-[calc(2rem+env(safe-area-inset-bottom))] sm:pb-7">
               ...
               <div className="space-y-3">
                 ...
                 <div className="grid grid-cols-3 gap-1.5">
                   {['15 分钟内', '15-45 分钟', '45 分钟以上'].map(t => (
                     <button key={t} className="py-3 rounded-[6px] bg-[#F5F5F7] border-black/5 text-[11px] font-bold text-[#1C1C1E] active:scale-95 transition-transform">{t}</button>
                   ))}
                 </div>
               </div>
               ...
               <div className="space-y-3">
                 ...
                 <div className="flex flex-wrap gap-1.5">
                   {['烤箱', '明火炉', '空气炸锅', '低温慢煮仪'].map(e => (
                     <button key={e} className="px-5 py-2.5 rounded-[6px] bg-[#F5F5F7] border-black/5 text-[11px] font-bold text-[#1C1C1E] active:scale-95 transition-transform">{e}</button>
                   ))}
                 </div>
               </div>
            </div>
            ...
            <button 
              onClick={() => setIsFilterOpen(false)}
              className="w-full bg-[#1C1C1E] text-white py-4 rounded-[6px] text-[13px] font-bold tracking-widest shadow-[0_8px_20px_rgba(0,0,0,0.15)] active:scale-[0.98] transition-all"
            >
              加载匹配档案
            </button>
          </div>
